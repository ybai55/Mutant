...
class GetDBTest(unittest.TestCase):
    ...
    def test_persistent_duckdb(self, mock):
        db = mutant.get_db(
            mutant.config.Settings(mutant_db_impl="duckdb+parquet", mutant_cache_dir="./foo")
        )
        assert mock.called
...
class GetAPITest(unittest.TestCase):
    ...
    def test_local(self, mock_api, mock_db):
        api = mutant.init(mutant.config.Settings(mutant_cache_dir="./foo"))
