...
def activation_uncertainty(training_data: pd.DataFrame, unlabeled_data: pd.DataFrame) -> np.ndarray:
    """Calculate the activation uncertainty of the unlabeled data, given the class-wise activation uncertainties in the
        training data.
    Args:
    ...
    for class_name in unlabeled_data["inference_class"].unique():
        ...
        activation_uncertainty[unlabeled_data["inference_class"] == class_name] = unlabeled_percentiles
...
def boundary_uncertainty(
    ...
    """Calculate the boundary uncertainty of the unlabeled data, the training data, and an ANN index containing both.
    Args:
        training_data (DataFrame): training data as a Pandas DataFrame.
    ...
    unlabeled_inference_categories = unlabeled_data["inference_class"].to_numpy()
    matches = neighbor_categories == unlabeled_inference_categories[:, np.newaxis]
...
def class_outliers(
    ...
    for class_name in unlabeled_data.inference_class.unique():
        ...
        representative_outliers[unlabeled_data["inference_class"] == class_name] = dist_diff_percentiles
        ...
        difficult_outliers[unlabeled_data["inference_class"] == class_name] = overall_median_percentiles
...
def cluster_outliers(
    ...
    Returns:
        Tuple[numpy NDArray, numpy NDArray]: Representative cluster-based outlier percentiles, Difficult cluster-based
                                            outlier percentiles, for the unlabeled data.
    """
