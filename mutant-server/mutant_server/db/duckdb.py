...
import uuid
import time
from mutant_server.db.clickhouse import (Clickhouse, db_array_schema_to_clickhouse_schema, EMBEDDING_TABLE_SCHEMA,
                                         RESULTS_TABLE_SCHEMA, db_schema_to_keys)
import pandas as pd
import numpy as np
...
class DuckDB(Clickhouse):
    ...
    def _create_table_results(self):
        self._conn.execute(f"""CREATE TABLE IF NOT results (
    ...
    # the execute many syntax is different thant clickhouse, the (?,?) syntax is different than clickhouse
    def add(self, model_space, embedding, input_uri, dataset=None, inference_class=None, label_class=None):
        data_to_insert = []
        for i in range(len(embedding)):
            data_to_insert.append([model_space[i], str(uuid.uuid4()), embedding[i], input_uri[i], dataset[i],
                                    inference_class[i], (label_class[i] if label_class is not None else None)])
    ...
    def _delete(self, where={}):
        ...
        self._conn.execute(f"""DELETE FROM embeddings {where}""").fetchall()[0]
    ...
    def get_by_ids(self, ids=list):
        ...
        return self._conn.execute(f"""SELECT {db_schema_to_keys()} FROM embeddings WHERE 
                uuid IN ({','.join([("'"+str(x)+"'") for x in ids])})""").fetchall()
