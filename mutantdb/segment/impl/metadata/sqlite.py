...
from mutantdb.types import Segment
from mutantdb.db.impl.sqlite import SqliteDB
from overrides import override
from mutantdb.db.base import (
    Cursor,
    ParameterValue,
    get_sql,
)
...
class SqliteMetadataSegment(MetadataReader):
    ...
    def _update_metadata(self, cur: Cursor, id: int, metadata: UpdateMetadata) -> None:
        ...
        if "mutant:document" in metadata:
    ...
    def _insert_metadata(self, cur: Cursor, id: int, metadata: UpdateMetadata) -> None:
        ...
        if "mutant:document" in metadata:
            ...
            q = (
                ...
                .insert(ParameterValue(id), ParameterValue(metadata["mutant:document"]))
            )
            sql, params = get_sql(q)
            cur.execute(sql, params)
