...
import logging
from mutantdb.telemetry import ServerContext, Telemetry
...
class FastAPI(mutantdb.server.Server):
    def __init__(self, settings: Settings):
        ...
        self._api: mutantdb.api.API = mutantdb.Client(settings)

        self._app.middleware("http")(catch_exceptions_middleware)
        self._app.add_middleware(
            CORSMiddleware,
            allow_headers=["*"],
            allow_origins=settings.mutant_server_cors_allow_origins,
            allow_methods=["*"],
        )
