...
from mutantdb.config import System, Settings
from mutantdb.types import (
...
from mutantdb.segment import MetadataReader
import uuid
import time

from mutantdb.segment.impl.metadata.sqlite import SqliteMetadataSegment
...
def sample_embeddings() -> Iterator[SubmitEmbeddingRecord]:
    def create_record(i: int) -> SubmitEmbeddingRecord:
        ...
        if i == 0:
            ...
            metadata["mutant:document"] = _build_document(i)
...
def test_fulltext(
    ...
    result = segment.get_metadata(where={"mutant:document": "four two"})
...
def _test_update(
    ...
    update_record = SubmitEmbeddingRecord(
        id="embedding_0",
        metadata={"mutant:document": "foo bar"},
        embedding=None,
        encoding=None,
        operation=op,
    )
    ...
    sync(segment, max_id)
    results = segment.get_metadata(ids=["embedding_0"])
    assert results[0]["metadata"] == {"mutant:document": "foo bar"}
    ...
    update_record = SubmitEmbeddingRecord(
        id="embedding_0",
        metadata={"mutant:document": "biz buz"},
        embedding=None,
        encoding=None,
        operation=op,
    )
    ...
    sync(segment, max_id)
    results = segment.get_metadata(ids=["embedding_0"])
    assert results[0]["metadata"] == {"mutant:document": "biz buz"}
    ...
    sync(segment, max_id)
    results = segment.get_metadata(ids=["embedding_0"])
    assert results[0]["metadata"] == {"mutant:document": "biz buz", "baz": 42}

    # Update and delete key
    update_record = SubmitEmbeddingRecord(
        id="embedding_0",
        metadata={"mutant:document": None},
        embedding=None,
        encoding=None,
        operation=op,
    )
