...
from mutantdb import config
from mutantdb.api import API
from mutantdb.api.types import Documents, EmbeddingFunction, Embeddings
import mutantdb.test.property.strategies as strategies
import mutantdb.test.property.invariants as invariants
...
def versions() -> List[str]:
    """Returns the pinned minimum version and the latest version of mutantdb."""
    url = "https://pypi.org/pypi/mutantdb/json"
...
def configurations(versions: List[str]) -> List[Tuple[str, Settings]]:
    return [
        (
            version,
            Settings(
                mutant_api_impl="mutantdb.api.segment.SegmentAPI",
                ...
                mutant_segment_manager_impl="mutantdb.segment.impl.manager.local.LocalSegmentManager",
                allow_reset=True,
                is_persistent=True,
                persist_directory=tempfile.gettempdir() + "/persistence_test_mutantdb",
            ),
        )
...
base_install_dir = tempfile.gettempdir() + "/persistence_test_mutantdb_versions"


# This fixture is not shared with the rest of the tests because it is unique in how it
# installs the versions of mutantdb
...
def get_path_to_version_library(version: str) -> str:
    return get_path_to_version_install(version) + "/mutantdb/__init__.py"
...
def switch_to_version(version: str) -> ModuleType:
    ...
    assert mutantdb.__version__ == version
    return mutantdb
